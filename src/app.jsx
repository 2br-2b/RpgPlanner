      ...
      <div data-theme={campaign.theme} className="sk-app" style={{ ...css.app, minHeight: "100dvh" }}>
        ...
        <div style={{ ...css.body, position: "relative", overflow: isMobile ? "visible" : "hidden" }}>
          ...
          <div className="sk-main" style={{ ...css.main, padding: mainPad, paddingBottom: isMobile ? "68px" : mainPad, overflowY: "auto" }}>
            ...
            {view === "editor" && selectedPage && <PageEditor key={selectedPage.id} page={selectedPage} schema={campaign.sectionSchema} onUpdate={(updater) => update((data) => ({ ...data, pages: data.pages.map((item) => item.id === selectedPageId ? updater(item) : item) }))} onBack={() => navigateTo("outline")} shareEnabled={campaign.shareEnabled || false} />}
