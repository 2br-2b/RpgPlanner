      ...
      {(sec.subheaders || []).length > 0
        ? (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 12 }}>
            {sec.subheaders.map(sh => (
              <SubBox key={sh} label={sh} value={getVal(sh)}
                onChange={v => onChange(sh, v)}
                onImageUpload={() => onImageUpload(sh)}
                expanded={expanded} T={T} css={css} />
            ))}
          </div>
        )
        : (
          <SubBox label={sec.name} value={flatVal}
